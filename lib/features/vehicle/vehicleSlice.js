...
export const vehicleSlice = createSlice({
  ...
  reducers: {
    ...
    resetVehicleToast: (state) => {
      state.toastMsg = null;
    },
    resetVehicleAddedToast: (state) => {
      state.vehicleAddedToast = null;
    },
    resetAddedToInv: (state) => {
      state.addedToInv = false;
    },
  },
  extraReducers: (builder) => {
    ...
    builder.addCase(addVehicle.fulfilled, (state, action) => {
      if (action.payload.success) {
        ...
        state.vehicleAddedToast = { msg: action.payload.message, red: false };
      } else {
        state.vehicleAddedToast = { msg: action.payload.message, red: true };
      }
    });
    ...
    builder.addCase(addVehicle.rejected, (state, action) => {
      if (action.payload) {
        state.error = action.payload;
        state.vehicleAddedToast = {
          msg: action?.payload?.response?.data?.message,
          red: true,
        };
      } else {
        state.vehicleAddedToast = "Conncetion Problem";
      }
    });
