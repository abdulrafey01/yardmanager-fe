...
import { updateCompany, updatePersonal } from "../../../../lib/features/profile/profileActions";
...
const page = () => {
	...
	const { toastMsg, companyData, personalData } = useSelector((state) => state.profile);
    ...
    let token = await getCookie("token");
		axios
    .put("https://yardmanager-be.vercel.app/api/users/company-image", imageForm, {
      headers: {
	...
	const onPersonalFormSubmit = (event) => {
		...
		if (personalFormState.password !== personalFormState.confirmPassword) {
			return dispatch(setShowToast({ value: true, msg: "Password don't match" }));
		}
	...
	return (
		...
		<div className="p-4 pr-6 md:pr-4 bg-[#f9fafb] relative flex-1 flex flex-col  space-y-4  w-screen md:w-full ">
			...
			<div className="w-full relative flex flex-col p-6 space-y-4">
				...
				{/* Block 1 */}
				<div className="w-full  bg-white p-4 space-y-4 rounded-lg" style={{ marginTop }}>
					<p className="font-bold text-[#344054] text-xl">Company Information</p>
					...
					<div className="flex w-full justify-end items-center space-x-4">
						<WhiteBtn
							onClick={() => setCompanyFormState({ name: "", address: "", phone: "" })}
							title={"Discard"}
						/>
				...
				<div className="w-full bg-white p-4 space-y-4 rounded-lg">
					<p className="font-bold text-[#344054] text-xl">Personal Information</p>
					<div className="grid grid-cols-2 w-full gap-4">
						<div className="w-full p-3 hover:border-gray-400 rounded-lg border border-[#D0D5DD]">
							<input
								className="w-full outline-none"
					...
					<div className="mt-4 space-y-4">
						<div className="flex justify-center items-center">
							<label
								...
								{image ? (
									<Image src={URL.createObjectURL(image)} width={100} height={100} alt="profile" className="w-40 h-40" />
								) : (
									<svg
										xmlns="http://www.w3.org/2000/svg"
										className="h-10 w-10"
						...
						<div className="flex justify-end">
							<div className="flex justify-center items-center">
								<button className="bg-[#78FFB6] text-black font-medium px-4 py-2 rounded-lg hover:bg-[#3FFB97]" onClick={uploadImage}>
									Save
								</button>
							</div>
						</div>
