...
const PartsPage = ({ isAdmin = false }) => {
  ...
  useEffect(() => {
    if (toastMsg?.red === false) {
      dispatch(fetchPartsByPage({ page: 1, limit: dataLimit, isAdmin }));
      setPageNumber(1);
    }
  }, [toastMsg]);
  // Search function
  ...
  const handleRadioClick = (e) => {
    if (e.target.value == 25) {
      dispatch(fetchPartsByPage({ page: 1, limit: 25, isAdmin }));
      setDataLimit(25);
      setPageNumber(1);
    } else if (e.target.value == 50) {
      dispatch(fetchPartsByPage({ page: 1, limit: 50, isAdmin }));
      setDataLimit(50);
      setPageNumber(1);
    } else {
